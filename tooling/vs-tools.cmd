...
if not '%VisualStudioVersion%'=='' (
    if not '%VisualStudioVersion%'=='17.0' (
        ...
        echo Run this command from a clean environment or from the 2022 x64 developer command prompt.
        exit /b 1
    )
...
for /f "usebackq tokens=*" %%i in (`call %VSWHERE% -latest -requires Microsoft.VisualStudio.Workload.NativeDesktop -version "[17.0,18.0)" -find VC\Auxiliary\Build\vcvars64.bat`) do (
    call "%%i" && exit /b 0
)

:: If we got this far, vswhere didn't return a result
echo Failed to find a compatible version of Visual Studio
echo Make sure you have a non-pre-release version of Visual Studio 2022 with the "Desktop development with C++" workload installed.
exit /b 1
